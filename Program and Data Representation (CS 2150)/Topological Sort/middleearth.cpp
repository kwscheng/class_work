...
/**
@brief Shuffles the string vector passed in.

@details This function shuffles string vectors by swapping random elements. A psuedorandom number is generated using the Mersenne Twister engine

@param first An iterator pointing to the first index of the range to be shuffled
@param last An iterator pointing to the last infex of the range to be shuffled
@param g A Marsenne-Twister random number engine
...
/**
 all_city_names is a string array of size 40. Holds the list of all the place names that we'll be using
 */
...
Creates a MiddleEarth object with city names pulled randomly from the all_city_names field. City positions are randomly computed based on the maximum "graph" dimensions. Inter-city distances stored in the 2-d distances field.

@param xsize "Length" of the MiddleEarth graph
@param ysize "Height" of the MiddleEarth graph
@param num_cities Number of desired cities
@param seed to use for random number generation
...
/**
  @brief Prints out info the created 'world'

@details Prints out number of total cities, and then the name and (x,y) position of all cities in the given MiddleEarth object.
...
/** @brief Prints a tab-separated table of the distances between each city

Outputted print can be saved as .tsv and exported to Excel and similar programs.
...
then this method call is also O(1) @emoji smile
@param city1 String name of first city 
@param city2 String name of second city
@return Returns a float representing the distance between city1 and city2
*/
...
/** @brief Returns the list of cities to travel to.
 ...
 @attention The first city is the original start point as well as the end point. The number of cities passed in does not include this start/end point (so there will be length+1 entries in the returned vector).
@param length The number of cities to travel to, not including start and end point
@return Returns a vector of strings representing the travel itinerary
*/
